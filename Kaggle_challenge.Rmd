...
```{r}
df = read.csv("kaggle_survey_2020_responses.csv", header = T)
...
setwd("C:/Users/as8126/Ankur/MSDS/msds_git/kaggle_challenge")

#headers = read.csv("cleanedsurveydata.csv", skip = 1, header = F, nrows = 1, as.is = T)
dfc = read.csv("cleanedsurveydata.csv",  header = T)
#colnames(dfc)= headers
...
dfc$highest.edu <- gsub('[^\x20-\x7E]', '', dfc$highest.edu)
...
```
```{r}
country_data <- joinCountryData2Map(dfc, joinCode='NAME', nameJoinColumn = "country")
...
```{r}
country_data <- joinCountryData2Map(df2, joinCode='NAME', nameJoinColumn = "country")

country_data
```

```{r}
country2 <- country_data %>%
  filter (is.na(country_data$counts))
...
```{r}
df_cord <- coordinates(country_data)

head(df_cord)
```

```{r}
df2 <- as.data.frame(df_cord)

df2
```

```{r}
df3 <- cbind(country = rownames(df2), df2)
...
```{r}
data.spatial <- df4 %>% #projecting the table as an sf and setting the coordinate system
...
leaflet()  %>%
  addTiles()  %>%
  addMarkers(data = data.spatial, popup = ~country)
```
...
dfc$lat[counter] <- 0
while (counter <= nrow){
  CityName <- gsub(' ','%20',dfc$CityLong[counter]) #remove space for URLs
